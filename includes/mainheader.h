...
// data structures
int							wifi_add_device_lst(t_ScannerContext *ctx,
								struct bss_info *bss);
int							bt_add_device_lst(t_ScannerContext *ctx,
								const char *mac_addr, int8_t rssi);
void						remove_device_lst(t_ScannerContext *ctx,
								void *device_to_remove, t_structype type);
void						free_devices_lst(t_ScannerContext *ctx,
								t_structype type);
