...
{
	if ((ctx->bt_dev_id = hci_get_route(NULL)) < 0) 			return -1;
    if ((ctx->bt_dev_fd = hci_open_dev(ctx->bt_dev_id)) < 0) 	return -1;
...
static int init_pthreads(t_AllThreads *t, t_ScannerContext *ctx)
{
	if (pthread_mutex_init(&ctx->ble_data_mutex, NULL) != 0
		...
		return (1);
	if (pthread_create(&t->ble_scan_thread, NULL, ble_scan_data_parser, ctx) != 0
		|| pthread_create(&t->dev_lst_monitor_thread, NULL, dev_lst_monitor, ctx) != 0
		|| pthread_create(&t->ble_send_thread, NULL, bt_senddata, ctx) != 0)
		return (1);
	if (ctx->wifi_scan_on)
	{
        if (pthread_mutex_init(&ctx->wifi_data_mutex, NULL) != 0 ||
            pthread_create(&t->wifi_scan_thread, NULL, wifi_scan_data_parser, ctx) != 0 ||
            pthread_create(&t->wifi_send_thread, NULL, wifi_senddata, ctx) != 0)
            return 1;
    }
	return (0);
}
...
int	main(int ac, char **av)
{
	...
	if (ctx.wifi_scan_on)
	{
		if (get_active_network_interface(ctx.wifi_iface, sizeof(ctx.wifi_iface)) != 0)
