...
static void add_update_device(t_ScannerContext *ctx, const char *mac_addr, int8_t rssi)
{
...
void print_device_info(const char *mac, int rssi, const uint8_t *data, size_t data_len) {
    ...
    while (index < data_len) {
        ...
        if (field_len == 0 || index + field_len >= data_len) break; // Safety check
        ...
        switch (field_type) {
            ...
            case 0xFF: // Manufacturer Specific Data
                if (field_len >= 3) { // Manufacturer ID plus at least one data byte
                    int manufacturer_id = data[index + 2] | (data[index + 3] << 8);
...
void	*ble_scan_data_parser(void *arg)
{
	...
	if (ble_hci_request(ctx, OCF_LE_SET_SCAN_PARAMETERS, LE_SET_SCAN_PARAMETERS_CP_SIZE, &scan_params_cp) < 0)
	...
	if (ble_hci_request(ctx, OCF_LE_SET_EVENT_MASK, LE_SET_EVENT_MASK_CP_SIZE, &event_mask_cp) < 0)
	...
	if (ble_hci_request(ctx, OCF_LE_SET_SCAN_ENABLE, LE_SET_SCAN_ENABLE_CP_SIZE, &scan_cp) < 0)
		...
		if (len >= HCI_EVENT_HDR_SIZE) {
			...
			if (meta_event->subevent == EVT_LE_ADVERTISING_REPORT) {
				...
				while (reports_count--) {
					...
					int8_t rssi = (int8_t) *(info->data + info->length);  // Assuming RSSI is at the end
					print_device_info(mac_addr, rssi, info->data, info->length);
					add_update_device(ctx, mac_addr, rssi);
					offset = (uint8_t *)offset + sizeof(le_advertising_info) + info->length + 1; // +1 for RSSI
				}
			}
		} else {
